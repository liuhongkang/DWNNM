function [ Y_hat, W_hat] = MCWNNM_ADMM2_Estimation( NL_mat, Sigma_arr, CurPat, Par )
...
for  i      =  1 : length(Par.SelfIndex) % For each keypatch group
    ...
    Y    =   Y-mY;
    X 	=   DWNNM_ADMM( Y, Sigma_arr(:, Par.SelfIndex(i)), Par); % DWNNM Estimation    取一列噪声强度Sigma_arr(:, Par.SelfIndex(i))
